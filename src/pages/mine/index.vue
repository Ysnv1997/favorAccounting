...
import { systeminfo } from '@/alicloud/services/system'
...
async function statistics() {
  const [giftOutRes, giftInRes,sysInfo_data] = await Promise.all([staticGiftOut(), staticGiftIn(),systeminfo()])
  if (giftOutRes.success)
    staticData.value.giftOut = giftOutRes.data

  if (giftInRes.success)
    staticData.value.giftIn = giftInRes.data
  if(sysInfo_data.success)
    sysinfoData.open_family  = sysInfo_data.result.open_family 
} 
onShareAppMessage(() => {
  return {
...
<template>
  <div class="pt-24 space-y-5">
	...
	<!-- 新增隐藏家人，以用于微信发布 -->
	<div v-if="sysinfoData.open_family" >
    ...
    <div class="rounded-2xl bg-white px-1 py-3 space-y-3">
      <uv-cell title="支持一下" url="/pages/sponsor/index" is-link :border="false">
        <template #icon>
