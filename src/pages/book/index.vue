	...
	<div>
		...
		<div class="grid grid-cols-2 gap-5 pt-6">
			<div v-for="i in books" :key="i._id" class="h-48 w-full rounded-l-5 rounded-r-10 bg-white"
				...
				<div class="relative">
					<div class="absolute bottom-22 right-0 h-7 w-18 flex items-center rounded-l-full"
						:class="[hasMourningWords(i.title) ? 'bg-gray' : 'bg-red']">
						<div class="ms-2 h-3 w-3 rounded-full"
							:class="[hasMourningWords(i.title) ? 'bg-gray-300' : 'bg-red-300']" />
					</div>
				</div>
			</div>
			<div class="h-48 w-full flex flex-col items-center justify-center rounded-l-5 rounded-r-10 bg-white"
